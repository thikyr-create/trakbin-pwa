...
import {
  ...
  ArrowRight, CheckCircle2, AlertCircle, Mail, Loader2, Sparkles,
} from 'lucide-react';
...
export default function CheckoutSheet({ open, mode, invoiceId, amount, description, onClose }: Props) {
  const { building, walletBalance, companyProfile, payInvoice } = useCaretakerSession();
    ...
    try {
      const res = await fetch('/api/payments/initialize', {
        ...
        body: JSON.stringify({
          ...
          purpose: mode === 'invoice' ? 'invoice' : 'topup',
      ...
      setError(FRIENDLY[e?.message] || e?.message || 'Something went wrong starting the payment.');
    ...
    <AnimatePresence>
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1000] flex items-end justify-center bg-black/55 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={loading ? undefined : onClose}>
          <motion.div
            initial={{ y: 40, opacity: 0, scale: 0.98 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 24, opacity: 0, scale: 0.98 }}
            transition={{ duration: 0.32, ease: EASE }} onClick={(e) => e.stopPropagation()}
            className={`${body.className} relative max-h-[92vh] w-full max-w-md overflow-hidden rounded-t-[28px] bg-white shadow-2xl sm:rounded-[28px]`}
          >
            {/* emerald console header */}
            <div className="relative overflow-hidden bg-emerald-950 px-6 pb-6 pt-5 text-white">
              ...
              {step !== 'done' && step !== 'error' && (
                <motion.p key={effectiveAmount} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={`${display.className} relative z-10 mt-1 text-5xl font-extrabold leading-none tracking-tight tabular-nums`}>
                  {formatNaira(effectiveAmount)}
                </motion.p>
              )}

              {mode === 'invoice' && step !== 'done' && step !== 'error' && (
                <p className="relative z-10 mt-2 text-xs font-medium text-emerald-200/70">to {providerName}</p>
              )}
            </div>
            ...
            <div className="max-h-[52vh] overflow-y-auto px-6 py-5">
              ...
              {step === 'method' && (
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, ease: EASE }} className="space-y-2.5">
                  ...
                  {methods.map((m, i) => {
                    ...
                    return (
                      <motion.button
                        key={m.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.32, delay: i * 0.05, ease: EASE }}
              ...
              {step === 'processing' && (
                <div className="flex flex-col items-center gap-4 py-8">
                  ...
                  <p className="text-sm font-bold text-gray-700">{method === 'wallet' ? 'Settling from wallet…' : 'Opening secure payment page…'}</p>
                </div>
              )}
              ...
              {step === 'done' && (
                <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="py-4 text-center">
                  ...
                  <p className="mt-1 text-sm font-medium text-gray-500">{formatNaira(effectiveAmount)} paid from your wallet.</p>
                </motion.div>
              )}
            ...
            <div className="border-t border-gray-100 bg-gray-50/70 px-6 py-4">
              {(step === 'amount' || step === 'method') && (
                <motion.button
                  ...
                  onClick={onContinue} disabled={!canContinue || loading}
                  className="group flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 py-3.5 text-sm font-extrabold text-white shadow-lg shadow-emerald-200 transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-gray-300 disabled:shadow-none"
                >
