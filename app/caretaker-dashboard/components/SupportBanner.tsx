...
import {
  ...
  Clock, Radio, ArrowUpRight, Building2, LifeBuoy, CalendarClock,
} from 'lucide-react';
...
export default function SupportBanner() {
    ...
    return (
      ...
      >
        <div
          ...
          style={{ backgroundImage: 'repeating-radial-gradient(circle at 88% -10%, rgba(255,255,255,0.5) 0 1px, transparent 1px 26px)' }}
        />
  ...
  // ── ACTIVE: identity + schedule + the live, multi-number contact console ─
  return (
    <motion.section
      initial={{ opacity: 0, y: 14 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      ...
      <div className="relative z-10 p-7">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="mb-1 flex items-center gap-2">
                ...
                {zone && (
                  <span className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-100/50">· Zone {zone}</span>
                )}
              </div>
        ...
        {/* schedule summary (folded in from the retired provider card) */}
        {(days.length > 0 || window) && (
          ...
          >
            <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
              <p className="mb-1 flex items-center gap-1.5 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-200/60">
                <CalendarClock className="h-3.5 w-3.5" /> Collection days
              </p>
        ...
        {/* contact actions — one chip per number, referenced from the live profile */}
        <div className="mt-7">
              ...
              {companyContacts.map((c, i) => {
                ...
                return (
                  <motion.a
                    ...
                    whileHover={{ y: -2 }}
                    whileTap={{ scale: 0.97 }}
                    className={`group relative flex items-center gap-3 overflow-hidden rounded-2xl bg-white p-3.5 text-left ring-1 ring-transparent transition-shadow duration-200 hover:shadow-lg hover:shadow-black/20 ${ring}`}
                  >
        ...
        <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-white/10 pt-5 text-xs text-emerald-100/70">
          ...
          <span className="inline-flex items-center gap-1.5"><Clock className="h-3.5 w-3.5 text-emerald-300" /> Avg response &lt; 4 hrs</span>
        </div>
      </div>
    </motion.section>
  );
