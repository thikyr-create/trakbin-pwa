...
import { Receipt, ArrowUpRight, Wallet, Plus, Activity } from 'lucide-react';
...
export default function BillingStatement() {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: EASE }} className="relative overflow-hidden rounded-[24px] border border-gray-200/80 bg-white p-7 shadow-sm sm:p-8">
      ...
      {ledger.length === 0 ? (
        ...
        <ol className="relative z-10 divide-y divide-gray-100">
          {ledger.map((t, i) => {
            ...
            return (
              <motion.li key={t.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.05 + i * 0.03, ease: EASE }} className="group flex items-center gap-4 py-4">
      ...
      <p className="relative z-10 mt-5 flex items-center gap-2 text-xs font-medium text-gray-400">
        <Activity className="h-3.5 w-3.5 text-emerald-500" /> Every top‑up and payment, recorded in order · balanced & immutable
      </p>
    </motion.section>
  );
}