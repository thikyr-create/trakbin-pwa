...
// Next upcoming pickup day from today, given a list of weekday names.
function nextPickup(days: string[]): { label: string; inDays: number } | null {
  ...
  for (let offset = 0; offset < 8; offset++) {
    ...
    if (set.has(WEEK[d.getDay()].toLowerCase())) {
      const label =
        offset === 0 ? 'Today' : offset === 1 ? 'Tomorrow' : WEEK[d.getDay()];
...
export default function ServiceVitalsCard() {
  ...
  const next = useMemo(() => nextPickup(days), [days.join(',')]);
  ...
  const vitals = [
    {
      Icon: CalendarCheck2,
      label: 'Service since',
      value: fmtDate(activeAssignment?.activated_at),
    ...
    },
    {
      ...
      value: next ? next.label : 'As scheduled',
      sub: next ? (next.inDays === 0 ? 'window open today' : `in ${next.inDays} day${next.inDays === 1 ? '' : 's'}`) : undefined,
      accent: 'text-emerald-700',
      live: true,
    },
    {
      Icon: Gauge,
      label: 'Collections on record',
      value: String(onRecord),
      sub: onRecord === 1 ? 'pickup logged' : 'pickups logged',
      accent: 'text-emerald-700',
    },
  ];
  ...
  return (
    ...
    >
      {/* ambient layered field — inverted vocabulary of the dark hero */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 opacity-[0.5]"
        style={{
          backgroundImage:
            'radial-gradient(circle at 100% 0%, rgba(16,185,129,0.07) 0 1px, transparent 1px), radial-gradient(circle at 0% 100%, rgba(16,185,129,0.05) 0 1px, transparent 1px)',
      ...
      {/* left signal rail that fills on mount */}
      <motion.span
        aria-hidden
        initial={{ scaleY: 0 }}
        animate={{ scaleY: 1 }}
        transition={{ duration: 0.7, delay: 0.15, ease: EASE }}
        className="absolute inset-y-5 left-0 w-1 origin-top rounded-r-full bg-gradient-to-b from-emerald-400 to-emerald-600"
      />
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700 ring-1 ring-emerald-200">
            ...
            Active{zone ? ` · Zone ${zone}` : ''}
          </span>
        </div>

        <h3 className="mt-2 text-2xl font-black leading-tight tracking-tight text-gray-900 sm:text-[26px]">
          Your collection, on the books
        </h3>
        <p className="mt-1 max-w-md text-sm font-medium text-gray-500">
          The operational facts of your active service — refreshed every time you open the app.
        </p>
          ...
          {vitals.map((v) => {
            ...
            return (
              <motion.div
                ...
                variants={cell}
                whileHover={{ y: -2 }}
                className="relative overflow-hidden rounded-2xl border border-gray-100 bg-gray-50/70 p-4 transition-colors duration-200 hover:border-emerald-200 hover:bg-emerald-50/40"
              >
                <div className="mb-3 flex items-center justify-between">
                  ...
                  {v.live && (
                    <motion.span
                      aria-hidden
                      className="flex items-center gap-1 font-mono text-[9px] font-bold uppercase tracking-[0.18em] text-emerald-500"
                ...
                <p className={`mt-0.5 text-lg font-black leading-tight tracking-tight ${v.accent}`}>
                  {v.value}
                </p>
