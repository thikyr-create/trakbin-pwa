...
export default function BottomSheet({ stop, isArrived, onComplete, onSkip, onNavigate, onReport }: BottomSheetProps) {
  if (!stop) {
    return (
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-400 flex items-center gap-1 mt-1">
              <MapPin size={12} /> {stop.address || 'Unknown Address'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
            ...
            <p className="text-sm font-black text-white mt-1">{stop.occupancy || 'N/A'}</p>
          </div>
          <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
            <p className="text-[10px] font-bold text-gray-400 uppercase flex items-center gap-1"><Package size={10} /> Est. Waste</p>
            <p className="text-sm font-black text-white mt-1">{stop.estimated_waste || 'N/A'}</p>
          </div>
          <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
          ...
          <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50">
            ...
            <p className="text-sm font-black text-white mt-1">{stop.waste_type || 'N/A'}</p>
          </div>
        </div>
        ...
        {isArrived ? (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                onClick={onNavigate}
                className="flex items-center justify-center gap-2 py-3 bg-blue-600/20 text-blue-400 font-bold rounded-xl text-sm uppercase hover:bg-blue-600/30 transition-colors border border-blue-600/50"
              >
                <Navigation size={16} /> Navigate
              </button>
              <button 
                onClick={onReport}
                className="flex items-center justify-center gap-2 py-3 bg-red-600/20 text-red-400 font-bold rounded-xl text-sm uppercase hover:bg-red-600/30 transition-colors border border-red-600/50"
              >
          ...
          <div className="space-y-3">
            <button 
              onClick={onNavigate}
              className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 text-white font-black rounded-xl text-sm uppercase hover:bg-blue-500 transition-colors shadow-lg shadow-blue-900/20"
            >
