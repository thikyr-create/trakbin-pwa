...
import { X, Search, MapPin, Building2, LocateFixed } from 'lucide-react';
...
export default function SearchScreen() {
  ...
  const {
    searchQuery, setSearchQuery, searchGeocode, geocodeResults, selectGeocodeResult, gpsLocation,
  } = useDriverSession();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100" style={{ paddingTop: 'calc(0.75rem + env(safe-area-inset-top))' }}>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3">
        {geocodeResults.length === 0 ? (
          <p className="text-center text-sm text-gray-400 py-10">
            {searchQuery.trim() ? 'No results found' : 'Search assigned stops, streets, landmarks…'}
          </p>
        ) : (
