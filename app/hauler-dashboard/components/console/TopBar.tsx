      ...
      <div className="flex items-center gap-3">
        ...
        <div
          className={`flex items-center gap-2 px-3 py-1.5 rounded-full font-bold text-xs uppercase tracking-wide ${
            onShift
              ? isRoutePaused ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
              : 'bg-gray-100 text-gray-500'
          }`}
        >
          <span className={`w-2 h-2 rounded-full ${onShift ? (isRoutePaused ? 'bg-amber-500' : 'bg-emerald-500 animate-pulse') : 'bg-gray-400'}`} />
          {onShift ? (isRoutePaused ? 'ON SHIFT · PAUSED' : 'ON SHIFT') : 'OFF SHIFT'}
        </div>
      </div>

      <div className="flex items-center gap-2">
        <button onClick={() => setNotifOpen(true)} className="relative p-2 hover:bg-gray-100 rounded-xl transition-colors" title="Notifications">
        ...
        <button
          onClick={() => { localStorage.removeItem('trakbin_driver'); window.location.href = '/'; }}
