...
import { Search, LogOut, Navigation } from 'lucide-react';
...
export default function HaulerDashboard() {
  ...
  // Show FAB when user scrolls away from current stop
  useEffect(() => {
    if (currentStop) {
      setShowFAB(true);
    }
  ...
  return (
    <div className="relative h-screen w-full bg-slate-900 overflow-hidden">
      ...
      <AnimatePresence>
        {showFAB && currentStop && (
          <motion.button
            ...
            className="absolute right-6 bottom-[100px] z-30 w-14 h-14 bg-emerald-600 hover:bg-emerald-500 rounded-full flex items-center justify-center shadow-2xl shadow-emerald-900/50 border-2 border-white"
          >
      ...
      {/* Bottom Panel (Unified & Slideable) */}
      <BottomPanel />
