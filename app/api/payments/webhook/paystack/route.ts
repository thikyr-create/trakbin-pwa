...
export async function POST(req: NextRequest) {
  ...
  if (event.event === 'charge.success') {
    ...
    const { data: payment, error: lockErr } = await admin().from('payments')
      .update({ status: 'processing', updated_at: new Date().toISOString() })
      .eq('reference', ref).eq('status', 'pending')
      .select('id, purpose, invoice_id, building_id, amount, provider, payer_email')
      .maybeSingle();
    ...
    if (verify.status !== 'success') {
      ...
      await markFailed(ref);
      await notify({
        emails: [payment.payer_email], buildingId: payment.building_id,
        type: 'charge_failed', title: 'Payment failed',
        body: `Your payment of ₦${Number(payment.amount).toLocaleString()} could not be completed.`,
        data: { reference: ref, amount: payment.amount, purpose: payment.purpose, reason: verify.status },
      });
      return NextResponse.json({ ok: true, status: 'failed' });
    }

    await handleSuccessfulPayment(verify, {
      purpose: payment.purpose, invoiceId: payment.invoice_id, buildingId: payment.building_id,
    });

    // ── Capture authorization (nested in verify.raw) ──
    ...
    if (auth?.authorization_code && payment.building_id) {
      ...
      const { data: existing } = await admin().from('payment_methods')
        .select('id').eq('building_id', payment.building_id).eq('card_last_four', last4).maybeSingle();
      ...
      if (existing) {
        ...
        await admin().from('payment_methods').insert({
          building_id: payment.building_id, instrument_type: 'card', type: 'card',
          provider: payment.provider || 'paystack', card_brand: brand, card_last_four: last4,
          authorization_code: authCode, authorization_reusable: reusable, is_default: false,
    ...
    const channel = (verify as any).channel ?? 'card';
    await notify({
      emails: [payment.payer_email], buildingId: payment.building_id,
      type: payment.purpose === 'topup' ? 'wallet_topup' : 'invoice_paid',
      title: payment.purpose === 'topup' ? 'Wallet funded' : 'Payment received',
      body: payment.purpose === 'topup'
        ? `₦${Number(payment.amount).toLocaleString()} added to your wallet via ${channel}.`
        : `₦${Number(payment.amount).toLocaleString()} payment confirmed.`,
      data: {
        reference: ref, amount: payment.amount, purpose: payment.purpose, channel,
        psp: payment.provider,
        card: auth ? { brand: auth.brand, last4: auth.last4 } : null,
      },
    });
  ...
  if (event.event === 'charge.failed') {
    const ref = event.data?.reference;
    if (!ref) return NextResponse.json({ ok: true });
    const { data: payment } = await admin().from('payments').select('payer_email, building_id, amount, purpose').eq('reference', ref).maybeSingle();
    await markFailed(ref);
    await notify({
      emails: payment?.payer_email ? [payment.payer_email] : [],
      buildingId: payment?.building_id ?? null,
      type: 'charge_failed', title: 'Payment failed',
      body: `Your payment of ₦${Number(payment?.amount ?? 0).toLocaleString()} could not be completed.`,
      data: { reference: ref, amount: payment?.amount ?? null, purpose: payment?.purpose ?? null },
    });
    return NextResponse.json({ ok: true, status: 'failed' });
  }

  // money-OUT: transfer events (payouts) — recipients need company→user mapping; wire once available
