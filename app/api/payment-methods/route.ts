...
export async function POST(req: NextRequest) {
  try {
    ...
    const row: any = {
      ...
      instrument_type: b.instrumentType,
    ...
    if (b.instrumentType === 'bank_account') {
      if (!b.bankCode || !b.accountLast4 || !b.accountName) return NextResponse.json({ ok: false, error: 'bank_details_required' }, { status: 400 });
      row.bank_code = b.bankCode;
      row.bank_name = b.bankName ?? null;
      row.account_last4 = b.accountLast4;
      row.account_name = b.accountName;
    }
    if (b.instrumentType === 'card') {
      row.card_last_four = b.cardLast4 ?? null;
      row.card_brand = b.cardBrand ?? 'card';
    }
