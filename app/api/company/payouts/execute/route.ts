...
export async function POST(req: NextRequest) {
  try {
    ...
    // Idempotent: already paid → success, no double execution
    ...
    const { error: updateErr } = await supabaseAdmin
      ...
      .eq('id', payoutId)
      .eq('status', 'pending'); // optimistic guard: only one executor wins

    if (updateErr) {
      return NextResponse.json({ ok: false, reason: updateErr.message }, { status: 500 });
    }

    // Move reserved balance to withdrawn
    ...
    if (hauler) {
      await supabaseAdmin
        .from('haulers')
        .update({
          pending_balance: Math.max(0, (hauler.pending_balance ?? 0) - payout.amount),
