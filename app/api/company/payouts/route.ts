...
export async function POST(req: NextRequest) {
  try {
    ...
    // Idempotency: same key → same result, never a duplicate payout
    ...
    if ((hauler.available_balance ?? 0) < amount) {
    ...
    // Reserve balance: available → pending
    await supabaseAdmin
      .from('haulers')
      .update({
        available_balance: (hauler.available_balance ?? 0) - amount,
        pending_balance: (hauler.pending_balance ?? 0) + amount,
      })
      .eq('id', companyId);
