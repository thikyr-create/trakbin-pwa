...
import { communications } from '@/lib/core/communications';
  ...
  try {
    ...
    const { data: driver, error: driverError } = await supabaseAdmin
      ...
      .insert({
        ...
        company_id,
        user_id: authUser.user.id, // ← the identity link the login flow requires
        status: 'active',
      })
      .select()
      .single();
    ...
    // 5) Credentials email via the communications engine (never Resend directly)
    const emailResult = await communications.sendDriverCredentials({
      email: driverEmail,
      driverName,
      employeeId,
      tempPassword,
      companyName: company_name || haulerRow.company_name || null,
    });
    ...
    return NextResponse.json(
      {
        ...
        emailSent: emailResult.status === 'sent' || emailResult.status === 'dry_run',
