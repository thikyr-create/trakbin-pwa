...
export default function CompanyIdCard({ company, onClose }: Props) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <motion.div initial={{ scale: 0.9, y: 20, rotate: -2 }} animate={{ scale: 1, y: 0, rotate: 0 }} transition={{ type: 'spring', stiffness: 240, damping: 18 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-sm overflow-hidden rounded-[24px] bg-gradient-to-br from-emerald-700 to-emerald-900 text-white shadow-2xl">
        <div className="relative p-6">
          ...
          <button onClick={onClose} className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white/80 hover:bg-white/20"><X size={18} /></button>
