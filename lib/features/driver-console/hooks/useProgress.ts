...
export function useProgress(): ProgressData {
  const { route, routeStops, driver, driverCompanyId } = useDriverSession();
  ...
  const stats = useMemo(() => {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (prev.latitude != null && prev.longitude != null && cur.latitude != null && cur.longitude != null) {
        traveledM += calculateDistanceInMeters(prev.latitude, prev.longitude, cur.latitude, cur.longitude);
      }
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (pauseOpenAt) pausedMs += Date.now() - new Date(pauseOpenAt).getTime(); // currently paused
