...
export async function initializePayment(args: InitializeArgs) {
  ...
  const init = await provider.initialize({
    amount: args.amount, currency: 'NGN', email: args.email,
    method: args.method as any, purpose: args.purpose, invoiceId: args.invoiceId,
    buildingId: args.buildingId, metadata: { callback_url: args.callbackUrl },
  });
...
// Fund the wallet from a SAVED, verified bank account via Paystack's bank-account charge.
// Server-only: the account number + name never leave the server. Idempotent via the PSP reference.
export async function chargeLinkedBank(args: { buildingId: string; methodId: string; amount: number; email: string }) {
  ...
  const init = await provider.initialize({
    amount: args.amount, currency: 'NGN', email: args.email, method: 'bank', purpose: 'topup',
    buildingId: args.buildingId,
    metadata: {
      callback_url: '', bank_account: method.account_number, account_name: method.account_name, bank_code: method.bank_code,
    },
  });

  await admin().from('payments').insert({
    provider: provider.name, reference: init.reference, building_id: args.buildingId,
    payer_email: args.email, purpose: 'topup', method: 'bank', channel: 'bank',
    amount: args.amount, currency: 'NGN', status: 'pending',
  });
