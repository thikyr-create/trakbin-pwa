...
export const authAdapter = {
  ...
  // dual-layer matcher — extracted verbatim from the auth page so it lives in one place
    ...
    for (const zone of zones) {
      ...
      // Layer 0: estate/street membership, only if the zone stores them
      const zoneEstates: string[] = Array.isArray(zone.estates) ? zone.estates : [];
      const zoneStreets: string[] = Array.isArray(zone.streets) ? zone.streets : [];
      if (estateLower && (zoneEstates.some((e) => String(e).toLowerCase() === estateLower) || zoneStreets.some((s) => String(s).toLowerCase() === estateLower))) {
        return zone.company_id;
      }
      // Layer 1: semantic text match (address + estate)
      const hay = estateLower ? `${addressLower} ${estateLower}` : addressLower;
      if (zoneNameLower && (hay.includes(zoneNameLower) || zoneNameLower.includes(addressLower.split(',')[0].trim()))) {
        return zone.company_id;
      }
